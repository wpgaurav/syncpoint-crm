...
if [[ $WP_VERSION =~ ^[0-9]+\.[0-9]+\-(beta|RC)[0-9]+$ ]]; then
	...
	# http serves a hierarchical list of all tags
	download http://api.wordpress.org/core/version-check/1.7/ /tmp/wp-latest.json
	grep -o '"version":"[^"]*' /tmp/wp-latest.json | sed 's/"version":"//' | head -1 > /tmp/wp-version.txt
	WP_VERSION=$(cat /tmp/wp-version.txt)
...
install_wp() {
	...
	download https://raw.githubusercontent.com/marber/wp-sqlite-integration/main/db.php $WP_CORE_DIR/wp-content/db.php
}
