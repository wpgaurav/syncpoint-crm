...
class SCRM_Admin {
	...
	public function render_settings() {
		...
		// Handle save.
		if ( isset( $_POST['scrm_save_settings'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'scrm_save_settings' ) ) {
			$this->save_settings( $tab );
			echo '<div class="notice notice-success"><p>' . esc_html__( 'Settings saved.', 'syncpoint-crm' ) . '</p></div>';
		}
		...
		<div class="wrap scrm-wrap">
			...
			<nav class="nav-tab-wrapper">
				<?php foreach ( $tabs as $slug => $label ) : ?>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=scrm-settings&tab=' . $slug ) ); ?>" class="nav-tab <?php echo $tab === $slug ? 'nav-tab-active' : ''; ?>">
			...
			<form method="post" style="max-width: 800px;">
				<?php wp_nonce_field( 'scrm_save_settings' ); ?>
				...
				<p class="submit">
					<button type="submit" name="scrm_save_settings" class="button button-primary"><?php esc_html_e( 'Save Settings', 'syncpoint-crm' ); ?></button>
				</p>
			</form>
		</div>
		<?php
	...
	private function render_general_settings() {
		...
		<table class="form-table">
			<tr>
				<th><label for="default_currency"><?php esc_html_e( 'Default Currency', 'syncpoint-crm' ); ?></label></th>
				<td>
					<select id="default_currency" name="default_currency">
						<?php foreach ( $currencies as $code => $data ) : ?>
							<option value="<?php echo esc_attr( $code ); ?>" <?php selected( $settings['default_currency'] ?? 'USD', $code ); ?>><?php echo esc_html( $code . ' - ' . $data['name'] ); ?></option>
						<?php endforeach; ?>
					</select>
				</td>
			</tr>
			<tr>
				<th><label for="contact_id_prefix"><?php esc_html_e( 'Contact ID Prefix', 'syncpoint-crm' ); ?></label></th>
				<td>
					<input type="text" id="contact_id_prefix" name="contact_id_prefix" value="<?php echo esc_attr( $settings['contact_id_prefix'] ?? 'CUST' ); ?>" class="regular-text">
					<p class="description"><?php esc_html_e( 'Prefix for auto-generated contact IDs (e.g., CUST-001).', 'syncpoint-crm' ); ?></p>
				</td>
			</tr>
			<tr>
				<th><label for="company_id_prefix"><?php esc_html_e( 'Company ID Prefix', 'syncpoint-crm' ); ?></label></th>
				<td>
					<input type="text" id="company_id_prefix" name="company_id_prefix" value="<?php echo esc_attr( $settings['company_id_prefix'] ?? 'COMP' ); ?>" class="regular-text">
				</td>
			</tr>
			<tr>
				<th><label for="date_format"><?php esc_html_e( 'Date Format', 'syncpoint-crm' ); ?></label></th>
				<td>
					<select id="date_format" name="date_format">
						<option value="M j, Y" <?php selected( $settings['date_format'] ?? 'M j, Y', 'M j, Y' ); ?>>Jan 1, 2026</option>
						<option value="Y-m-d" <?php selected( $settings['date_format'] ?? '', 'Y-m-d' ); ?>>2026-01-01</option>
						<option value="d/m/Y" <?php selected( $settings['date_format'] ?? '', 'd/m/Y' ); ?>>01/01/2026</option>
					</select>
				</td>
			</tr>
		</table>
	...
	private function render_paypal_settings() {
		...
		<table class="form-table">
			<tr>
				<th><label for="paypal_enabled"><?php esc_html_e( 'Enable PayPal', 'syncpoint-crm' ); ?></label></th>
				<td><input type="checkbox" id="paypal_enabled" name="paypal_enabled" value="1" <?php checked( $settings['enabled'] ?? false ); ?>></td>
			</tr>
			<tr>
				<th><label for="paypal_mode"><?php esc_html_e( 'Mode', 'syncpoint-crm' ); ?></label></th>
				<td>
					<select id="paypal_mode" name="paypal_mode">
						<option value="sandbox" <?php selected( $settings['mode'] ?? 'sandbox', 'sandbox' ); ?>><?php esc_html_e( 'Sandbox (Testing)', 'syncpoint-crm' ); ?></option>
						<option value="live" <?php selected( $settings['mode'] ?? '', 'live' ); ?>><?php esc_html_e( 'Live', 'syncpoint-crm' ); ?></option>
					</select>
				</td>
			</tr>
			<tr>
				<th><label for="paypal_client_id"><?php esc_html_e( 'Client ID', 'syncpoint-crm' ); ?></label></th>
				<td><input type="text" id="paypal_client_id" name="paypal_client_id" value="<?php echo esc_attr( $settings['client_id'] ?? '' ); ?>" class="large-text"></td>
			</tr>
			<tr>
				<th><label for="paypal_client_secret"><?php esc_html_e( 'Client Secret', 'syncpoint-crm' ); ?></label></th>
				<td><input type="password" id="paypal_client_secret" name="paypal_client_secret" value="<?php echo esc_attr( $settings['client_secret'] ?? '' ); ?>" class="large-text"></td>
			</tr>
		</table>
		<?php
	}
	...
	private function render_stripe_settings() {
		...
		<table class="form-table">
			<tr>
				<th><label for="stripe_enabled"><?php esc_html_e( 'Enable Stripe', 'syncpoint-crm' ); ?></label></th>
				<td><input type="checkbox" id="stripe_enabled" name="stripe_enabled" value="1" <?php checked( $settings['enabled'] ?? false ); ?>></td>
			</tr>
			<tr>
				<th><label for="stripe_mode"><?php esc_html_e( 'Mode', 'syncpoint-crm' ); ?></label></th>
				<td>
					<select id="stripe_mode" name="stripe_mode">
						<option value="test" <?php selected( $settings['mode'] ?? 'test', 'test' ); ?>><?php esc_html_e( 'Test', 'syncpoint-crm' ); ?></option>
						<option value="live" <?php selected( $settings['mode'] ?? '', 'live' ); ?>><?php esc_html_e( 'Live', 'syncpoint-crm' ); ?></option>
					</select>
				</td>
			</tr>
			<tr>
				<th><label for="stripe_test_pk"><?php esc_html_e( 'Test Publishable Key', 'syncpoint-crm' ); ?></label></th>
				<td><input type="text" id="stripe_test_pk" name="stripe_test_pk" value="<?php echo esc_attr( $settings['test_publishable_key'] ?? '' ); ?>" class="large-text"></td>
			</tr>
			<tr>
				<th><label for="stripe_test_sk"><?php esc_html_e( 'Test Secret Key', 'syncpoint-crm' ); ?></label></th>
				<td><input type="password" id="stripe_test_sk" name="stripe_test_sk" value="<?php echo esc_attr( $settings['test_secret_key'] ?? '' ); ?>" class="large-text"></td>
			</tr>
			<tr>
				<th><label for="stripe_live_pk"><?php esc_html_e( 'Live Publishable Key', 'syncpoint-crm' ); ?></label></th>
				<td><input type="text" id="stripe_live_pk" name="stripe_live_pk" value="<?php echo esc_attr( $settings['live_publishable_key'] ?? '' ); ?>" class="large-text"></td>
			</tr>
			<tr>
				<th><label for="stripe_live_sk"><?php esc_html_e( 'Live Secret Key', 'syncpoint-crm' ); ?></label></th>
				<td><input type="password" id="stripe_live_sk" name="stripe_live_sk" value="<?php echo esc_attr( $settings['live_secret_key'] ?? '' ); ?>" class="large-text"></td>
			</tr>
		</table>
		<?php
	}
	 ...
	 * @since 1.0.0
	 */
	private function render_invoice_settings() {
		$settings = scrm_get_settings( 'invoices' );
		?>
		<table class="form-table">
			<tr>
				<th><label for="invoice_company_name"><?php esc_html_e( 'Company Name', 'syncpoint-crm' ); ?></label></th>
				<td><input type="text" id="invoice_company_name" name="invoice_company_name" value="<?php echo esc_attr( $settings['company_name'] ?? get_bloginfo( 'name' ) ); ?>" class="large-text"></td>
			</tr>
			<tr>
				<th><label for="invoice_company_address"><?php esc_html_e( 'Company Address', 'syncpoint-crm' ); ?></label></th>
				<td><textarea id="invoice_company_address" name="invoice_company_address" rows="3" class="large-text"><?php echo esc_textarea( $settings['company_address'] ?? '' ); ?></textarea></td>
			</tr>
			<tr>
				<th><label for="invoice_tax_id"><?php esc_html_e( 'Tax ID / VAT Number', 'syncpoint-crm' ); ?></label></th>
				<td><input type="text" id="invoice_tax_id" name="invoice_tax_id" value="<?php echo esc_attr( $settings['company_tax_id'] ?? '' ); ?>" class="regular-text"></td>
			</tr>
			<tr>
				<th><label for="invoice_default_terms"><?php esc_html_e( 'Default Terms & Conditions', 'syncpoint-crm' ); ?></label></th>
				<td><textarea id="invoice_default_terms" name="invoice_default_terms" rows="4" class="large-text"><?php echo esc_textarea( $settings['default_terms'] ?? '' ); ?></textarea></td>
			</tr>
			<tr>
				<th><label for="invoice_due_days"><?php esc_html_e( 'Default Due Days', 'syncpoint-crm' ); ?></label></th>
				<td>
					<input type="number" id="invoice_due_days" name="invoice_due_days" value="<?php echo esc_attr( $settings['default_due_days'] ?? 30 ); ?>" min="1" style="width: 80px;">
					<p class="description"><?php esc_html_e( 'Number of days until invoice is due.', 'syncpoint-crm' ); ?></p>
				</td>
			</tr>
		</table>
		<?php
	...
	private function render_webhook_settings() {
		...
		$webhook_url = rest_url( 'starter-crm/v1/webhooks/inbound' );
		?>
		<table class="form-table">
			<tr>
				<th><label for="webhook_enabled"><?php esc_html_e( 'Enable Webhooks', 'syncpoint-crm' ); ?></label></th>
				<td><input type="checkbox" id="webhook_enabled" name="webhook_enabled" value="1" <?php checked( $settings['enabled'] ?? true ); ?>></td>
			</tr>
			<tr>
				<th><?php esc_html_e( 'Webhook URL', 'syncpoint-crm' ); ?></th>
				<td>
					<code style="display: block; padding: 10px; background: #f6f7f7;"><?php echo esc_html( $webhook_url ); ?></code>
					<p class="description"><?php esc_html_e( 'Use this URL to receive data from external services.', 'syncpoint-crm' ); ?></p>
				</td>
			</tr>
			<tr>
				<th><?php esc_html_e( 'Webhook Secret Key', 'syncpoint-crm' ); ?></th>
				<td>
					<code style="display: block; padding: 10px; background: #f6f7f7;"><?php echo esc_html( $settings['secret_key'] ?? 'Not set' ); ?></code>
					<p class="description"><?php esc_html_e( 'Include this key in the request to authenticate.', 'syncpoint-crm' ); ?></p>
				</td>
			</tr>
			<tr>
				<th><label for="webhook_ip_whitelist"><?php esc_html_e( 'IP Whitelist', 'syncpoint-crm' ); ?></label></th>
				<td>
					<textarea id="webhook_ip_whitelist" name="webhook_ip_whitelist" rows="3" class="regular-text" placeholder="<?php esc_attr_e( 'One IP per line', 'syncpoint-crm' ); ?>"><?php echo esc_textarea( $settings['ip_whitelist'] ?? '' ); ?></textarea>
					<p class="description"><?php esc_html_e( 'Optional. Leave empty to allow all IPs.', 'syncpoint-crm' ); ?></p>
				</td>
			</tr>
		</table>
		<?php
		...
		switch ( $tab ) {
			case 'general':
				$settings = array(
					...
					'contact_id_prefix'  => sanitize_text_field( $_POST['contact_id_prefix'] ?? 'CUST' ),
					'company_id_prefix'  => sanitize_text_field( $_POST['company_id_prefix'] ?? 'COMP' ),
					'date_format'        => sanitize_text_field( $_POST['date_format'] ?? 'M j, Y' ),
				);
				break;
				
			case 'paypal':
				$settings = array(
					'enabled'       => ! empty( $_POST['paypal_enabled'] ),
					'mode'          => sanitize_text_field( $_POST['paypal_mode'] ?? 'sandbox' ),
					'client_id'     => sanitize_text_field( $_POST['paypal_client_id'] ?? '' ),
					'client_secret' => sanitize_text_field( $_POST['paypal_client_secret'] ?? '' ),
				);
				break;
			...
			case 'stripe':
				$settings = array(
					...
					'test_publishable_key' => sanitize_text_field( $_POST['stripe_test_pk'] ?? '' ),
					'test_secret_key'      => sanitize_text_field( $_POST['stripe_test_sk'] ?? '' ),
					'live_publishable_key' => sanitize_text_field( $_POST['stripe_live_pk'] ?? '' ),
					'live_secret_key'      => sanitize_text_field( $_POST['stripe_live_sk'] ?? '' ),
				);
				break;
				
			case 'invoices':
				$settings = array(
					'company_name'     => sanitize_text_field( $_POST['invoice_company_name'] ?? '' ),
					'company_address'  => sanitize_textarea_field( $_POST['invoice_company_address'] ?? '' ),
					'company_tax_id'   => sanitize_text_field( $_POST['invoice_tax_id'] ?? '' ),
					'default_terms'    => sanitize_textarea_field( $_POST['invoice_default_terms'] ?? '' ),
					'default_due_days' => absint( $_POST['invoice_due_days'] ?? 30 ),
				);
				break;
			...
			case 'webhooks':
				$settings = array(
					...
					'ip_whitelist' => sanitize_textarea_field( $_POST['webhook_ip_whitelist'] ?? '' ),
				);
				// Preserve existing secret key.
				$existing = scrm_get_settings( 'webhooks' );
				$settings['secret_key'] = $existing['secret_key'] ?? wp_generate_password( 32, false );
				break;
		}
		
		update_option( 'scrm_settings_' . $tab, $settings );
	}
}
