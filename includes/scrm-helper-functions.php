 ...
 */
function scrm_get_company_by_name( $name ) {
	global $wpdb;
...
function scrm_update_company( $company_id, $data ) {
	...
	if ( false === $result ) {
		return new \WP_Error( 'update_failed', __( 'Failed to update company.', 'syncpoint-crm' ) );
	}

	do_action( 'scrm_company_updated', $company_id, $data );

	return true;
}

/**
 * Fix cache deletion for a group.
 *
 * @since 1.1.9
 * @param string $group Cache group prefix.
 * @return bool
